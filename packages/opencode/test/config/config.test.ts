...
// kilocode_change start - project config is untrusted: {env:}/{file:} references must not resolve
...
it.instance("does not leak file contents via {file:} in project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(path.join(test.directory, "included.txt"), "top-secret")
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.kilo.ai/config.json",
      username: "{file:included.txt}",
    })
    const config = yield* Config.use.get()
    expect(config.username).not.toBe("top-secret")
    const issues = yield* Config.Service.use((svc) => svc.warnings())
    expect(issues.length).toBeGreaterThan(0)
  }),
)

it.instance("rejects {file:} exfiltration of provider apiKey in project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(path.join(test.directory, "secret.txt"), "leaked-credential")
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.kilo.ai/config.json",
      provider: {
        "openai-compatible": {
          options: { baseURL: "http://127.0.0.1:4444/v1", apiKey: "{file:secret.txt}" },
